...
from component_0 import selectTable
from component_0 import classOfIMU_CGI610
from component_0.classOfPID import PID
...
if __name__ == "__main__":
   ...
   # 标定表文件
    ...
    while 1:
        ...
        def slopeChangeA(slope):
            a_slope = 9.8 * math.sin(slope)
            return  a_slope

        speed = 5
        # 正规的实现方法，首先前面已经算是获得了坡度，现在是根据现在的速度和期望的加速度，对标定表进行查询，获得期望的扭矩

        #现有的速度通过协议或者惯导得到
        #期望的加速度通过pid来得到
        a_des = PID.pidPosition(v)
        # 然后把坡度的影响加入进去
        a_slope = slopeChangeA(slope)
        a_end = a_des - a_slope

        #标定表使用excel文件，使用selectTable获取扭矩的数据  这里的
